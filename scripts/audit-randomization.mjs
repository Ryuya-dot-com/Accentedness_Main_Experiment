...
const DESIGN_INPUT = Object.freeze({
  ...
  randomizationSecret: "offline-audit-secret-not-used-for-data-collection",
});
...
for (let id = 1; id <= MAX_ID; id += 1) {
  const design = await buildParticipantDesign({
    participantId: id,
    participantUuid: uuidForId(id),
    ...DESIGN_INPUT,
  });
  designs.push(design);
  const expectedAccent = ["japanese", "english", "chinese"][id % 3];
  assert.equal(design.assignment.trainingAccent, expectedAccent, `ID ${id}: accent remainder`);
  assert(!roots.has(design.assignment.rootSeedHex), `ID ${id}: root seed collision`);
  roots.add(design.assignment.rootSeedHex);

  const learning = mainTrials(design, "immediate", "learning");
  assert.equal(learning.length, 144, `ID ${id}: learning count`);
  for (let itemId = 1; itemId <= 24; itemId += 1) {
    const exposures = learning.filter((trial) => trial.itemId === itemId);
    assert.equal(exposures.length, 6, `ID ${id}, item ${itemId}: exposures`);
    assert.equal(
      new Set(exposures.map((trial) => trial.talkerId)).size,
      exposures[0].variability === "no" ? 1 : 6,
      `ID ${id}, item ${itemId}: learning talker contract`,
    );
  }
  for (let index = 1; index < learning.length; index += 1) {
    if (learning[index - 1].variability === "high" && learning[index].variability === "high") {
      assert.notEqual(learning[index - 1].talkerId, learning[index].talkerId, `ID ${id}: adjacent High talker`);
    }
  }

  const prePn = mainTrials(design, "pre", "picture_naming");
  assert.equal(prePn.length, 24, `ID ${id}: pre Picture Naming count`);
  assert.equal(design.pre.trials.filter((trial) => trial.segment === "l2_to_l1").length, 0, `ID ${id}: no pre L2`);
  worstVariabilityRun = Math.max(worstVariabilityRun, maxRun(prePn.map((trial) => trial.variability)));
  assert(maxRun(prePn.map((trial) => trial.variability)) <= 2, `ID ${id}: pre Picture Naming variability run`);
  ...
  for (const visitType of ["immediate", "delayed"]) {
    ...
    assert(worstVariabilityRun <= 2, `ID ${id}: Picture Naming variability run`);
    ...
    assert(
      ...
      `ID ${id}: fixed test talker per accent`,
    );
    ...
    assert(fullAccentRun <= 2, `ID ${id}: full L2 accent run`);
    assert(maxRun(l2.map((trial) => trial.variability)) <= 2, `ID ${id}: L2 variability run`);
    for (const variability of ["no", "high"]) {
      for (const accent of ["english", "chinese", "japanese"]) {
        ...
        assert.deepEqual(sortedValues(count(talkers)), [4], `ID ${id}: ${accent}/${variability} fixed talker`);
      }
    }
  }
  ...
  assert.notDeepEqual(pnOrders[0], pnOrders[1], `ID ${id}: pre/immediate PN order`);
  assert.notDeepEqual(pnOrders[0], pnOrders[2], `ID ${id}: pre/delayed PN order`);
  assert.notDeepEqual(pnOrders[1], pnOrders[2], `ID ${id}: immediate/delayed PN order`);
  assert.notDeepEqual(
    mainTrials(design, "immediate", "l2_to_l1").map((trial) => trial.itemId),
    mainTrials(design, "delayed", "l2_to_l1").map((trial) => trial.itemId),
    `ID ${id}: L2 timepoint order`,
  );
}

const first72 = designs.slice(0, 72);
assert.deepEqual(sortedValues(count(first72.map((design) => design.assignment.trainingAccent))), [24, 24, 24]);
for (const accent of ["english", "chinese", "japanese"]) {
  ...
  assert.deepEqual(new Set(joint.values()), new Set([1]), `${accent}: N=216 joint cells`);
}
