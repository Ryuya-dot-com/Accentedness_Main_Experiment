...
export async function requireSession(request, env, { allowCompleted = false } = {}) {
  const token = bearerToken(request);
  ...
  if (session.session_status !== "active" && !completedRetry) {
  ...
  if (!completedRetry && nowMs - Number(session.session_last_seen_at_ms ?? 0) >= 60_000) {
